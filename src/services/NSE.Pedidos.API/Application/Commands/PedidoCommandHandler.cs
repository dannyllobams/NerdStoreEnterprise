...
namespace NSE.Pedidos.API.Application.Commands
{
        ...
        IRequestHandler<AdicionarPedidoCommand, ValidationResult>
    {
        ...
        public async Task<ValidationResult> Handle(AdicionarPedidoCommand message, CancellationToken cancellationToken)
        {
            ...
            if (!await ValidarPedido(pedido))
                return ValidationResult;

            if (!await ProcessarPagamento(pedido))
                return ValidationResult;

            pedido.AutorizarPedido();
            pedido.AdicionarEvento(new Events.PedidoRealizadoEvent(pedido.Id, pedido.ClienteId));
        ...
        private async Task<bool> ProcessarPagamento(Pedido pedido)
        {
            return await Task.FromResult(true);
        }
    }
